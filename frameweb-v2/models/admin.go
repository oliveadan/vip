...
type Admin struct {
	...
	LoginVerify       int8                                          // 验证状态(0:无验证;1:邮箱验证；2：谷歌安全码验证;)
...
func (model *Admin) Paginate(page int, limit int, adminRefId int64, param1 string) (list []Admin, total int64) {
	if page < 1 {
		page = 1
	}
	offset := (page - 1) * limit
	...
	if adminRefId != 0 {
		cond = cond.And("RefId", adminRefId)
	}
	if param1 != "" {
